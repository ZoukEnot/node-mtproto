import bigInt from 'big-integer';
import { Serializer } from '../tl/serializer';
import { SHA1 } from './sha1';
import { bytesToHex, hexToBytes, bigIntToBytes, bytesToBigInt } from '../utils';

interface PublicKey {
  modulus: string;
  exponent: string;
}

const publisKeys: PublicKey[] = [
  {
    modulus:
      '',
    exponent: '010001',
  },
  {
    modulus:
      '',
    exponent: '010001',
  },
  {
    modulus:
      '',
    exponent: '010001',
  },
  {
    modulus:
      '',
    exponent: '010001',
  },
  {
    modulus:
      '',
    exponent: '010001',
  },
];

interface RSAPublicKey {
  modulus: string;
  exponent: string;
}

interface RSAPublicKeyWithFingerprint extends RSAPublicKey {
  fingerprint: bigInt.BigInteger;
}

export class RSA {

  async getPublisKeysByHex(): Promise<{
    [fingerprintHex: string]: RSAPublicKey;
  }> {
    const publisKeysByHex: { [fingerprintHex: string]: RSAPublicKey } = {};

    for (const publisKey of publisKeys) {
      const RSAPublicKey = new Serializer(function (this: Serializer) {
        this.bytes(hexToBytes(publisKey.modulus));
        this.bytes(hexToBytes(publisKey.exponent));
      });

      const buffer = RSAPublicKey.getBuffer();

      // @ts-ignore
      const fingerprintBytes = (await SHA1(buffer)).slice(-8);
      fingerprintBytes.reverse();

      publisKeysByHex[bytesToHex(fingerprintBytes)] = {
        modulus: publisKey.modulus,
        exponent: publisKey.exponent,
      };
    }

    return publisKeysByHex;
  }

  async getRsaKeyByFingerprints(
    fingerprints: bigInt.BigInteger[],
  ): Promise<RSAPublicKeyWithFingerprint | null> {
    let resultKey: RSAPublicKeyWithFingerprint | null = null;
    const publisKeysByHex = await this.getPublisKeysByHex();

    fingerprints.forEach((fingerprint) => {
      if (resultKey) {
        return;
      }

      const fingerprintHex = bigInt(fingerprint).toString(16);

      const foundKey = publisKeysByHex[fingerprintHex];

      if (foundKey) {
        resultKey = {
          fingerprint,
          ...foundKey,
        };
      }
    });

    return resultKey;
  }

  encrypt(publicKey: RSAPublicKey, bytes: Uint8Array): Uint8Array {
    const encryptedBigInt = bytesToBigInt(bytes).modPow(
      bigInt(publicKey.exponent, 16),
      bigInt(publicKey.modulus, 16),
    );

    return bigIntToBytes(encryptedBigInt, 256);
  }
}

export const rsa = new RSA();
