...
import { getRandomBytes } from 'src/crypto/get-random-bytes';
import { AES } from '../crypto/aes';

// @ts-ignore
type CTR = AES.CTR

export class ObfuscatedTransport extends EventEmitter {
  encryptAES: CTR;
  decryptAES: CTR;

  async generateObfuscationKeys(): Promise<Uint8Array> {
    ...
    while (!completed) {
      ...
      if ([0x44414548, 0x54534f50, 0x20544547, 0x4954504f, 0xdddddddd, 0xeeeeeeee, 0x02010316].includes(firstInt)) {
        continue;
      }
  ...
  async obfuscate(bytes: Uint8Array): Promise<Uint8Array> {
    return this.encryptAES.encrypt(bytes);
  }

  async deobfuscate(bytes: Uint8Array): Promise<Uint8Array> {
    return this.decryptAES.decrypt(bytes);
  }

  getIntermediatePayload(bytes: Uint8Array): Uint8Array {
    const dataView = new DataView(bytes.buffer);
    const payloadLength = dataView.getUint32(0, true);

    if (payloadLength === 4) {
      const code = dataView.getInt32(4, true) * -1;
      this.emit('error', { type: 'transport', code });
    }
